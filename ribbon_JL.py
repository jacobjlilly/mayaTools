...
# This script creates a ribbon with 6 bind joints and 3 driver joints
# Jacob Lilly

# Create ribbon geo (nurbs plane) and driver joints
geo = cmds.nurbsPlane(n='ribbon_GEO', ax=(0,1,0),lr=0.1666666667, d=3, w=6, u=6, v=1, ch=0)
cmds.makeIdentity(geo, a=1, t=1, r=1, s=1)
cmds.DeleteHistory()
cmds.select(cl=1)
drv1 = cmds.joint(n='ribbon_01_DRV', p=(0,0,0))
cmds.select(cl=1)
drv2 = cmds.joint(n='ribbon_02_DRV', p=(-3,0,0))
cmds.select(cl=1)
drv3 = cmds.joint(n='ribbon_03_DRV', p=(3,0,0))
drvs = [drv1, drv2, drv3]
...
cmds.select(geo)
mel.eval('createHair 6 1 10 0 0 0 0 5 0 2 2 1;')
